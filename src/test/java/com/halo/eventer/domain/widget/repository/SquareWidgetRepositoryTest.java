...
@Transactional
public class SquareWidgetRepositoryTest {
    ...
    void SquareWidget_리스트_festivalId_로_조회(){
        ...
        //when
        List<SquareWidget> squareWidgets = squareWidgetRepository.findAllByFestivalId(festival.getId());

        //then
        assertThat(squareWidgets).hasSize(5);
    }

    private void saveDownWidget(int count) {
        for (int i = 0; i < count; i++) {
            SquareWidget squareWidget = WidgetFixture.정사각형_위젯_엔티티(festival, squareWidgetCreateDto);
            setField(squareWidget,"updatedAt", LocalDateTime.now());
            squareWidgetRepository.save(squareWidget);
        }
    }
}
