...
public class StampUserAdminControllerTest {
    ...
    @MockitoBean StampUserAdminService service;
    ...
    private StampUserDetailResDto 샘플_상세() {
        ...
        return new StampUserDetailResDto(사용자_ID, "김수한", "010-1234-5678", 사용자_UUID, false, missions);
    }
    ...
    class 목록_조회 {
        ...
        void 성공() throws Exception {
            ...
            mockMvc.perform(get("/api/v2/admin/festivals/{festivalId}/stamp-tours/{stampId}/users",
                            축제_ID, 스탬프_ID)
                            .param("q", "김")
    ...
    class 상세_조회 {
        ...
        void 성공() throws Exception {
            ...
            mockMvc.perform(get("/api/v2/admin/festivals/{festivalId}/stamp-tours/{stampId}/users/{uuid}",
                            축제_ID, 스탬프_ID, 사용자_UUID)
                            .header(HttpHeaders.AUTHORIZATION, AUTH))
                    .andExpect(status().isOk())
    ...
    class 전체미션_완료_토글 {
        ...
        void 성공_complete_true() throws Exception {
            given(service.setAllMissionsCompletion(축제_ID, 스탬프_ID, 사용자_ID, true))
                    .willReturn(샘플_상세());

            var body = Map.of("complete", true);

            mockMvc.perform(patch("/api/v2/admin/festivals/{festivalId}/stamp-tours/{stampId}/users/{userId}/missions/all",
                            축제_ID, 스탬프_ID, 사용자_ID)
                            .header(HttpHeaders.AUTHORIZATION, AUTH)
