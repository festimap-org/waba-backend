...
public class StampUserAdminDocs {
    ...
    public static RestDocumentationResultHandler listUsers() {
        return document("v2-stampuser-list",
                resource(builder()
                        .tag(TAG)
                        ...
                        .queryParameters(
                                parameterWithName("q").description("검색어(이름/전화번호, 공백이면 전체)").optional(),
                                parameterWithName("missionCleared").description("미션 완료여부 필터 (ALL|TRUE|FALSE)").optional(),
                                parameterWithName("page").description("페이지(0-base)").optional(),
                                parameterWithName("size").description("페이지 크기").optional(),
                                parameterWithName("sortType").description("정렬 기준 (NAME|CREATED_AT)").optional()
                        )
                        .responseFields(
                                ...
                                fieldWithPath("content[].finished").type(BOOLEAN).description("투어 완료 여부"),
                                fieldWithPath("pageInfo").type(OBJECT).description("페이지 정보"),
                                fieldWithPath("pageInfo.pageNumber").type(NUMBER).description("현재 페이지(1-base 문서화 예시)"),
                                fieldWithPath("pageInfo.pageSize").type(NUMBER).description("페이지 크기"),
                                fieldWithPath("pageInfo.totalElements").type(NUMBER).description("전체 개수"),
    ...
    public static RestDocumentationResultHandler getUser() {
        return document("v2-stampuser-get",
                resource(builder()
                        .tag(TAG)
                        .summary("사용자 상세 조회")
                        .pathParameters(
                        ...
                        .responseFields(
                                ...
                                fieldWithPath("missions[].userMissionId").type(NUMBER).description("사용자-미션 ID"),
                                fieldWithPath("missions[].missionId").type(NUMBER).description("미션 ID"),
                                fieldWithPath("missions[].missionTitle").type(STRING).description("미션 제목"),
                                fieldWithPath("missions[].complete").type(BOOLEAN).description("완료 여부")
                        )
                        .build()));
    }

    public static RestDocumentationResultHandler setAllMissionsCompletion() {
        return document("v2-stampuser-missions-all",
                resource(builder()
                        .tag(TAG)
