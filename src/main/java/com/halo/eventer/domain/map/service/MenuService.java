...
public class MenuService {
    ...
    public List<MenuResDto> update(Long mapId, List<MenuUpdateDto> menuUpdateDtos) {
        List<Menu> menus = menuRepository.findAllByIdIn(menuUpdateDtos.stream().map(MenuUpdateDto::getId)
                .collect(Collectors.toList()));
        java.util.Map<Long, Menu> menuMap = menus.stream()
                .collect(Collectors.toMap(Menu::getId, Function.identity()));
        menuUpdateDtos.forEach(menuUpdateDto ->{
            menuMap.get(menuUpdateDto.getId())
                    .updateMenu(menuUpdateDto);
        });
