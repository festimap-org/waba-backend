...
public class SmsService {
  ...
  public void sendSms(MissingPersonReqDto dto, List<String> phoneList) throws Exception {
    ...
    if (dto.getBase64Image() != null) {
      FileUploadResDto fileUploadResDto =
          smsRequest.sendFileUpload("thumbnail1.jpeg", dto.getBase64Image());
      fileDtos.add(new FileDto(fileUploadResDto.getFileId()));
    }

    String realMessage =
        String.format(
            message, dto.getName(), dto.getAge(), dto.getGender(), dto.getMissingLocation());

    // 2. 관리자 리스트 수만큼 MessageDto setter로 to 필드 변경하기
    List<MessageDto> messages = new ArrayList<>();
    for (String phone : phoneList) {
      MessageDto messageDto = MessageDto.builder().to(phone).content(realMessage).build();
